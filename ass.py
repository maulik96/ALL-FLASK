	...
	for fileName in fileNames :
		...
		for line in lines :
			...
			if var.match(line):
				...
				extTable[fileName][ext.match(line).group(1).lstrip().rstrip()] = '$'+str(ext.match(line).group(1).lstrip().rstrip())
			elif add.match(line):
				...
				if tryInt(x):
					...
					newCode.append('JZ start'+str(loopctr))
					memaddr += oplen['PUSH']
					memaddr += oplen['MVI']
					memaddr += oplen['MOV']
					memaddr += oplen['JNZ']
				...
				newCode.append('JNZ end'+str(loopctr-1))
		...
		for line in lines :
			if '&&&' in line:
				...
				ifTable[fileName]['&&&'+str(linenum)]= newtag

			elif 'start' in line:
				tag = line.split(' ')[1]
				linenum = tag.split('start')[1].lstrip().rstrip()
				linenum = int(linenum)
				newtag = '#'+str(loopjmp[linenum])
				newCode.append(line.replace(tag, newtag))
				ifTable[fileName]['start'+str(linenum)]= newtag


			elif 'end' in line:
				tag = line.split(' ')[1]
				linenum = tag.split('end')[1].lstrip().rstrip()
				linenum = int(linenum)
				newtag = str(symTable[fileName][linenum])
				newCode.append(line.replace(tag, newtag))
				ifTable[fileName]['end'+str(linenum)]= newtag
